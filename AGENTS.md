...
Branch `features/read-path-qos`. Goal: MCP read latency p95 ~1s while indexing runs
(observed p95 15s+). Read path gets dedicated high QoS; indexing is best-effort,
throttled, background QoS. Ports the dotfiles codesearch governor into the product.

### Root causes (surveyed)
1. `SharedStores.vector_store: Arc<tokio::RwLock<VectorStore>>` -- inserts, deletes and
   `build_index` take the write lock; tokio RwLock is fair, so every search queues
   behind a pending writer for the whole txn. FSW runs `build_index` per changed file.
2. Inserts commit before the build; until build commits, `Reader::open` returns
   NeedBuild and `indexed=false` -> semantic search errors, hybrid silently drops to FTS.
3. `fts_store` write lock held across tantivy commit + reader reload.
4. One tokio runtime; ONNX (all cores per session, new session per batch/file), arroy
   (global rayon) and tantivy saturate CPU at default priority; reads do CPU work on
   the same workers.
5. No cross-repo indexing limit; queries trigger cold open + build + refresh inline;
   `warmup_repo` holds `open_lock` through a full refresh.
6. Embedding cache env opened outside `lmdb_registry` -> "double-open prevented",
   indexer runs cacheless.
7. Watcher ignore matcher != walker (root-only .gitignore, built once).

### Design: CQRS over LMDB MVCC
- **Query side**: readers take no app lock. LMDB read txns see the last committed
  snapshot; tantivy `Searcher` is a snapshot. Readers never wait on writers.
- **Command side**: one writer per store, serialized by an internal mutex. Every
  mutation publishes atomically: delete + insert + incremental arroy build in ONE
  write txn (arroy 0.8 build is incremental), so no reader ever observes NeedBuild.
  Only `env.resize` (rare MDB_MAP_FULL) excludes readers, via an internal gate.
- **Execution isolation**: reads run on a dedicated high-QoS pool
  (macOS USER_INITIATED); indexing on a dedicated low-QoS pool (UTILITY/BACKGROUND,
  Linux nice) with capped ONNX intra-op threads and arroy under `pool.install`.
- **Scheduler**: all indexing entry points submit jobs to `IndexScheduler`
  (single worker by default, priority queue, per-alias dedupe) gated on memory
  pressure, CPU idle, AC power and read-latency feedback (pause while reads are slow).
- **Telemetry**: in-process latency recorder for every tool call, p50/p95/p98/p99/p100,
  exposed in /status and the `status` tool; feeds the scheduler gate.

### Phases (one commit series each, tests first)
1. Lock-free reads: VectorStore/FtsStore interior mutability, `Arc<VectorStore>`,
   `Arc<FtsStore>`, atomic publish, drop per-file build, release `open_lock` early.
2. Execution isolation + QoS pools, shared indexing embedder, shared cache env.
3. `IndexScheduler` with gates, backoff, job timeout; all entry points routed.
4. Latency telemetry + SLO in /status and `status` tool.
5. Write-path hygiene: watcher ignore parity + prune, tantivy merge policy,
   git hook queue semantics + root filter.
